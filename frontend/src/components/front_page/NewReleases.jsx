...
const NewReleases = () => {
    ...
    const fetchProducts = async () => {
        try{
            ...
            setProducts(response.filter(product => product.category === 'sonic').slice(0, 6))
        }catch(e){
            console.log(e)
        }
    }
  ...
  {products.map((product, i) => (
    <div key={i} className='border border-gray-200 rounded-lg overflow-hidden'>
      ...
      <div className='p-4'>
        ...
        <button onClick={() => handleAddToCart(product)} className='text-sm w-full p-2 mt-4 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 transition-colors cursor-pointer'>
          Add to Cart
        </button>
      </div>
    </div>
